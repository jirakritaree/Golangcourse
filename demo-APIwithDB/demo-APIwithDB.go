...
func handlerCourses(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		CourseID, err := insertCourse(course) //Add Data to Database
		if err != nil {
			log.Print(err)
			w.WriteHeader(http.StatusBadRequest)
			return
...
func handlerCourse(w http.ResponseWriter, r *http.Request) {
	...
	switch r.Method {
	case http.MethodGet:
		course, err := getCourse(courseID) // Pull Data in ID
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
...
func corsMiddleware(handler http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Add("Access-Control-Allow-Headers", "Origin, Accept, Content-Type, Content-Length, Authorization, X-Cross, X-Requested-With")
